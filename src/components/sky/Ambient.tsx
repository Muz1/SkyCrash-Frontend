...
import { ascentLayers, skinMood, type SkinId } from "./SkyEnvironment";
...
function Layer({ items, opacity = 1 }: { items: Drifter[]; opacity?: number }) {
  if (opacity <= 0.01) return null;
  return (
    <div className="absolute inset-0 transition-opacity duration-700 ease-linear" style={{ opacity }}>
      {items.map((it, i) => (
        <div
          key={i}
          className="absolute"
        ...
        >
          <div style={{ animation: `ambient-bob ${it.bob}s ease-in-out infinite` }}>
            <img
              ...
              height={512}
              loading="lazy"
              className="drop-shadow-[0_0_18px_color-mix(in_oklab,var(--neon-blue)_60%,transparent)]"
              style={{
                width: it.size,
                height: "auto",
...
/** Slow background life: drifting clouds low down, planets and astronauts up high. */
export function Ambient({ skin, progress = 0 }: { skin: SkinId; progress?: number }) {
  const ascent = skin === "taking-off";
  const { space } = ascentLayers(progress);
  const mood = skinMood(skin);
  const skyOpacity = ascent ? 1 - space : mood === "space" ? 0 : 1;
  const spaceOpacity = ascent ? space : mood === "space" ? 1 : 0;

  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
      <Layer items={SKY} opacity={skyOpacity} />
      <Layer items={SPACE} opacity={spaceOpacity} />
    </div>
  );
}
