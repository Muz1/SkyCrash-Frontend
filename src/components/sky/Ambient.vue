...
import { ascentLayers, skinMood, type SkinId } from '@/lib/skins'
...
const skyOpacity = computed(() => (ascent.value ? 1 - layers.value.space : mood.value === 'space' ? 0 : 1))
const spaceOpacity = computed(() => (ascent.value ? layers.value.space : mood.value === 'space' ? 1 : 0))
</script>
