...
import { ascentLayers, skinMood, skinSrc, type SkinId } from "@/lib/skyEnvironments";
...
const props = withDefaults(
  ...
  { skin: "sunset-runway", showGrid: true, dim: 0.35, priority: false, progress: 0 },
);
...
const cloudsVisible = computed(() => (ascent.value ? true : mood.value !== "space"));
const cloudsOpacity = computed(() => (ascent.value ? 1 - layers.value.space : 1));

/** Crossfading backdrop: any skin change fades the previous image out. */
...
watch(
  () => props.skin,
  (nextSkin, prevSkin) => {
    if (nextSkin === prevSkin || nextSkin === "taking-off") return;
    seq += 1;
    const id = seq;
    crossfadeLayers.value = [...crossfadeLayers.value.slice(-2), { id, src: skinSrc(nextSkin) }];
    setTimeout(() => {
...
const dimGradient = computed(
  () =>
    `linear-gradient(180deg, oklch(0.11 0.06 285 / ${props.dim + 0.25}) 0%, oklch(0.11 0.06 285 / ${props.dim}) 45%, oklch(0.11 0.06 285 / ${Math.min(props.dim + 0.5, 0.95)}) 100%)`,
);
</script>
...
<template>
  <div aria-hidden="true" class="absolute inset-0 overflow-hidden">
    ...
    <div aria-hidden="true" class="absolute inset-0 transition-opacity duration-700" :style="{ opacity: starsOpacity }">
      <span
        v-for="(s, i) in stars"
        :key="i"
    ...
    <div
      v-if="cloudsVisible"
      aria-hidden="true"
      class="absolute inset-x-0 bottom-0 top-1/3 overflow-hidden transition-opacity duration-500"
    ...
    <div
      ...
      :style="{ opacity: ascent ? 1 - layers.cloud : mood === 'ground' ? 1 : 0.25 }"
    />

    <div class="absolute inset-0 transition-opacity duration-300" :style="{ background: dimGradient }" />
    <div
