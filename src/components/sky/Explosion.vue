...
const smokes = Array.from({ length: 7 }, (_, i) => i)
</script>
...
<template>
  ...
  >
    ...
    <span
      ...
      :key="delay"
      :class="cn('absolute inset-0 rounded-full', i % 2 ? 'border-4 border-danger' : 'border-4 border-ember')"
    ...
    <span
      ...
      :style="{
        width: `${px * (1 + (i % 3)) * 0.6}px`,
        height: `${px * (1 + (i % 3)) * 0.6}px`,
        ...
        '--dx': `${Math.cos((i / 24) * Math.PI * 2 + (i % 3) * 0.12) * (size * (0.42 + (i % 4) * 0.16))}px`,
        '--dy': `${Math.sin((i / 24) * Math.PI * 2 + (i % 3) * 0.12) * (size * (0.42 + (i % 4) * 0.16))}px`,
        '--rot': `${(i % 2 ? 1 : -1) * 320}deg`,
    ...
    <span
      ...
      :style="{
        ...
        '--dx': `${Math.cos((i / 7) * Math.PI * 2) * size * 0.32}px`,
        '--dy': `${Math.sin((i / 7) * Math.PI * 2) * size * 0.32 - size * 0.1}px`,
